...
export default function Scene({}: Props) {
  ...
  useGSAP(() => {
    ...
    gsap.set(can1Ref.current.rotation, { z: -0.5 }); // blackCherry

    gsap.set(can2Ref.current.position, { x: 1.5 }); // lemonLime
    gsap.set(can2Ref.current.rotation, { z: 0.5 }); // lemonLime

    gsap.set(can3Ref.current.position, { y: 5, z: 2 }); // grape
    gsap.set(can4Ref.current.position, { x: 2, y: 4, z: 2 }); // strawberryLemonade
    gsap.set(can5Ref.current.position, { y: -5 }); // watermelon
